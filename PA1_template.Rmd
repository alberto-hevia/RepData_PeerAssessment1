...
``` {r uploading-data, echo=TRUE, results = "hide", message=FALSE, warnings=FALSE}
setInternet2(TRUE)
...
``` {r stepsPerDay, echo = FALSE, fig.height = 5, fig.width=11, fig.path = 'figures/'}
print(p)
...
``` {r averageDaily, fig.path = 'figures/'}
...
```
Interval with the maximum steps is `r max.interval` with `r max.steps` steps.
...
Total number of missing values in the dataset is `r num.NAs`
...
``` {r histogramTotalSteps, echo = FALSE, fig.height = 5, fig.width = 11,fig.path = 'figures/'}
print(p)
```
...
``` {r plot-5-minute, fig.path = 'figures/'}
